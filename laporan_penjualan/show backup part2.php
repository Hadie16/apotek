...
<div class="table-responsive mt-3" >
          <table class="table table-bordered table-hover" id="viewReturObat" style="width: 100%;">
            ...
            <tbody>
              ...
              // GROUP BY month";
              //========obat with join several table=========
                // $monthlyQuery = "SELECT SUM(po.total_harga) AS monthly_sum, SUM(ko.harga_beli_obat) AS monthly_sum_beli,  MONTH(po.tanggal_penjualan_obat) AS month
                // FROM penjualan_obat as po 
                // join detail_penjualan_obat as dpo on po.id_penjualan_obat=dpo.id_penjualan_obat 
                // join stok_obat as so on dpo.id_stok_obat=so.id_stok_obat
                // join detail_stok_obat as dso on so.id_stok_obat=dso.id_stok_obat
                // join ketersediaan_obat as ko on dso.id_ketersediaan_obat=ko.id_ketersediaan_obat
                // GROUP BY month";
//========= join with id_obat====================
// $currentMonths = date('n');

              //   $monthlyQuery = "SELECT SUM(po.total_harga) AS monthly_sum, dpo.jumlah_detail_penjualan_obat as jm,ko.harga_beli_obat as hr ,SUM(ko.harga_beli_obat) AS monthly_sum_beli,  MONTH(po.tanggal_penjualan_obat) AS month
              //   FROM penjualan_obat as po 
              //   join detail_penjualan_obat as dpo on po.id_penjualan_obat=dpo.id_penjualan_obat 
              //   join ketersediaan_obat as ko on dpo.id_obat=ko.id_obat
              //  ";
//               $monthlyQuery = "SELECT * from penjualan_obat
//              ";

// $monthlyResult = mysqli_query($con, $monthlyQuery);

// while ($rows = mysqli_fetch_assoc($monthlyResult)) {
//  $jm[$rows['month']] = $rows['jm'];
//  $hr[$rows['month']] = $rows['hr'];
...
//   $monthlySumBeli[$row['month']] = $row['monthly_sum_beli'];
//   // $jm[$rows['month']] = $rows['jm'];
//   // $hr[$rows['month']] = $rows['hr'];
...
// $currentMonthSumBeli = $monthlySumBeli[$currentMonth];

// $jmhr = $jm[$currentMonth]*$hr[$currentMonth];
...
$monthlyResult = mysqli_query($con, $monthlyQuery);
              ...
              <tr>
                ...
                <td><?php echo $data['harga_detail_penjualan_obat']; ?></td>
                <td><?php echo $data['harga_beli_obat']; ?></td>            
                <td><?php echo $data['harga_detail_penjualan_obat']-$data['harga_beli_obat'] ; ?></td>


                <td><?php echo $data['jumlah_detail_penjualan_obat']; ?></td>
                <td><?php echo $v1 = $data['harga_detail_penjualan_obat']*$data['jumlah_detail_penjualan_obat']; ?></td>
                <td><?php echo $v2 = $data['harga_beli_obat']*$data['jumlah_detail_penjualan_obat']; ?></td>
