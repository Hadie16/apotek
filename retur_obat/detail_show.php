  ...
  <div class="col">
    <div class="card shadow mb-4">
      <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
        <h6 class="m-0 font-weight-bold text-info">Detail retur Obat</h6>
        <?php
        // $query = mysqli_query($con,  "SELECT a.*,b.kode_penjualan_obat kode,b.tanggal_penjualan_obat tanggal FROM detail_penjualan_obat a join penjualan_obat b on a.id_penjualan_obat=b.id_penjualan_obat  WHERE a.id_penjualan_obat=$id ");
           $query = mysqli_query($con,  "SELECT * FROM retur_obat WHERE id_retur_obat=$id limit 1");
        if (!$query) {
      ...
      <div class="card-body">
        <!-- <a href="?page=detail_retur_obat-add" class="btn btn-sm btn-success"><i class="fas fa-plus"></i> Tambah Data</a> -->
        <!-- <a href="../detail_retur_obat/print.php" class="btn btn-sm btn-info" target="_blank"><i class="fas fa-print"></i>
          Cetak
          FPDF</a> -->
        <!-- <a href="../detail_retur_obat/print2.php" class="btn btn-sm btn-warning" target="_blank"><i class="fas fa-print"></i>
          Cetak</a> -->

          <a href="?page=retur_obat-show" class="btn btn-sm btn-danger"><i class="fas fa-chevron-left"></i>
          Kembali</a>
        <hr>
        ...
        <div class="table-responsive mt-3" >
          <table class="table table-bordered table-hover" id="viewDSO" style="width: 100%;">
            ...
            <tbody>
              ...
              $query = mysqli_query($con,"SELECT a.*, c.nama_obat FROM detail_retur_obat a JOIN obat c ON a.id_obat = c.id_obat where id_retur_obat=$id");
        //     if ($query) {
...
$no=1;
 ...
 <td><?php echo $data['batch_number']; ?></td>
                ...
                <a class="btn text-info" href="?page=retur_obat-edit&id=<?php echo $data['id_retur_obat']; ?>"><i
                  ...
                  <a class="btn text-danger" href="?page=retur_obat-detail_delete&id=<?php echo $data['id_retur_obat']; ?>"
                    onclick="return confirm('Anda yakin mau menghapus item ini ?')"><i class="fas fa-trash"></i>
                  </a>
                <a class="btn text-success" href="../detail_retur_obat/print3.php?id=<?php echo $data['id_detail_retur_obat']; ?>"
