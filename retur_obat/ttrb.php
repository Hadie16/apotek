...
// include '../connection.php';
      ...
      include '../connection.php';
      $querys = mysqli_query($con, "SELECT po.*,s.nama_supplier from retur_obat po join detail_retur_obat dpo on po.id_retur_obat=dpo.id_retur_obat join obat o on dpo.id_obat=o.id_obat join supplier s on po.id_supplier=s.id_supplier where po.id_retur_obat='$id'");
      if ($querys) {
...
<p style="margin-left: 35px;">No. <?php echo $kodes?></p>
  <br>
<p style="margin-left: 35px;">Dengan ini kami mengembalikan obat-obatan, sebagai berikut :</p>
...
// $id = $_GET['id'];
      ...
      // include '../connection.php';
      $query = mysqli_query($con, "SELECT * from retur_obat po join detail_retur_obat dpo on po.id_retur_obat=dpo.id_retur_obat join obat o on dpo.id_obat=o.id_obat where po.id_retur_obat='$id'");
      if ($query) {
        // echo "<p>query berhasil<p/>";
    } else {
        die('invalid Query : ' . mysqli_error($con));
...
<div align="center" style="position: relative; top: 50px; left: 500px;">
  <p>Martapura,
     <?php
          $query1 = mysqli_query($con, "SELECT tanggal_retur from retur_obat where id_retur_obat='$id'");
          if ($query1) {
            // echo "<p>query berhasil<p/>";
        } else {
            die('invalid Query : ' . mysqli_error($con));
