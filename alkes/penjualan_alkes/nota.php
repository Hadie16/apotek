    ...
    <?php 
       include '../connection.php';
           $id = $_GET['id'];
                    ...
                    $query3 = mysqli_query($con, "SELECT * FROM penjualan_alkes WHERE id_penjualan_alkes=$id ");
                 
                    while ($rowDatas = mysqli_fetch_assoc($query3)) {
                      $kode = $rowDatas['kode_penjualan_alkes'];
                      $tgl = $rowDatas['tanggal_penjualan_alkes'];
    ...
    <table>
      ...
      <?php
          //  $id = $_GET['id'];
              ...
              $query = mysqli_query($con,"SELECT a.*, c.nama_alkes nama FROM detail_penjualan_alkes a JOIN stok_alkes b ON a.id_stok_alkes = b.id_stok_alkes JOIN alkes c ON b.id_alkes = c.id_alkes WHERE a.id_penjualan_alkes=$id");
...
$no=1;
        ...
        <td><?php echo $no++?></td>
                ...
                <td class="text-nowrap"><?php echo $data['jumlah_detail_penjualan_alkes']; ?></td>
                <td class="text-nowrap"><?php echo $data['satuan']; ?></td>
                <td>
  <span style="float: left;">Rp</span>
  <span style="float: right;"><?php echo number_format($data['harga_detail_penjualan_alkes'], 0, ',', '.'); ?></span>
</td>

               
                <td>
  <span style="float: left;">Rp</span>
  <span style="float: right;"><?php echo number_format($data['total_harga_detail_penjualan_alkes'], 0, ',', '.'); ?></span>
</td>
              </tr>
      ...
      <?php 
                    $query2 = mysqli_query($con, "SELECT * FROM penjualan_alkes  WHERE id_penjualan_alkes=$id ");
