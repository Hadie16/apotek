...
$varP = $_GET['id1'];
// $kodeP = $_GET['id2'];
$queryLog = "SELECT * FROM retur_obat WHERE id_retur_obat = $varP";
...
if (isset($_POST['submit'])) {
  ...
  VALUES('$kode_penerimaan_obat','$no_faktur','$tanggal_penerimaan_obat',0,'$id_supplier')");
  if (!$insert) {
  ...
  // ============== ==== ===  update status retur
 $updatePO = mysqli_query($con, "UPDATE retur_obat SET status='Selesai' WHERE id_retur_obat=$varP");
  ...
  for ($i = 0; $i < count($id_detailPO_array); $i++) {
   ...
   $boxsatuan= $boxsatuan_array[$i];
    ...
    $insert_query = "INSERT INTO detail_penerimaan_obat (id_penerimaan_obat,id_obat, jumlah_detail_penerimaan_obat,satuan,tanggal_kadaluarsa,batch_number,harga_detail_penerimaan_obat,sub_total) 
                   VALUES (' $firstTableID','$id_obat','$jumlah','$satuan','$tanggal_exp','$batch_number',0,0)";
...
$query = mysqli_query($con, "SELECT * FROM retur_obat WHERE id_retur_obat=$varP");
...
?>
                ...
                <input type="hidden" class="form-control" name="id_retur_obat" value="<?php echo $varP; ?>" readonly>

                <input type="text" class="form-control" name="no_faktur"  placeholder="Nomer Faktur" value="<?php echo $faktur ?>" required>
                ...
                <tr>
                  ...
                  <td width="15%">
                    <select name="id_detailPO[]" id="" class="form-control select-options select-option2" required>
                      ...
                      $query = mysqli_query($con, "SELECT a.*,b.nama_obat nama FROM detail_retur_obat a join obat b on a.id_obat=b.id_obat where id_retur_obat=$varP");
        
                      while ($row = mysqli_fetch_assoc($query)) {
                        // $id_obat1 = $row['id_obat'];
                        $id_D = $row['id_detail_retur_obat'] . "retur";
                        $nama = $row['nama'];
