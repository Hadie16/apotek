...
<script>
  $(document).ready(function() {
    ...
    $('#id_obat_select_retur').on('change', function() {
      var selectedOption = $(this).val();
   
      // Make AJAX request to fetch data based on selected option
      $.ajax({
        url: '../retur_obat/fetch_retur.php', // PHP script to retrieve data from table_1
        type: 'POST',
        ...
        success: function(response) {
          ...
          if (response.status === 'success') {
            ...
            $('#id_obats_retur').val(response.data.id_obat);
      ...
      $('#jumlah_retur_obat').on('input', function() {
      var jumlah = parseInt($('#jumlah_ketersediaan_obat').val());
        var jumlah_retur_obat = parseInt($(this).val());
        var jumlah_ketersediaan_obat = jumlah - jumlah_retur_obat;

        if (jumlah_retur_obat > jumlah) {
          $('#jumlah_retur_obat').val(jumlah);
}
